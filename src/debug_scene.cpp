...
boost::optional<boost::shared_ptr<Error> > DebugScene::SceneInitialize(void) {
	...
	OPT_ERROR(AddUI(base_window, box, uis::UIBase::MOVE_MODE_FREE_FREE, 10, 25, 620, 445));
	OPT_ERROR(AddTextUI(base_window, box, boost::shared_ptr<std::string>(new std::string("城郭都市「ジークフロント」")), uis::UIBase::MOVE_MODE_CENTER_FREE, 262, 9, 116, 32));
...
#undef ADD_SELECT
	...
	OPT_ERROR(AddPTStatusUI(base_window, box, pt, uis::UIBase::MOVE_MODE_FREE_FREE, 0, 350, 640, 130));
	return boost::none;
}
