...
boost::optional<boost::shared_ptr<Error> > OpeningWindow::Draw(void) {
	...
	if(frame_counter == 0) {
		::DrawString(80,380,"いざ、一攫千金に沸く城郭都市へ",0xFFFFFFFF);
		::DrawString(85,400,"天龍の塔への挑戦ははじまった",0xFFFFFFFF);
	}
	if(frame_counter == 300) {
		::DrawString(265,90,"それは、青天を貫く天空への旅",0xFFFFFFFF);
		::DrawString(235,110,"神に迫らんとするバベルを極めん道のり",0xFFFFFFFF);
	}
	if(frame_counter == 600) {
		::DrawString(325,300,"その意味すら知らぬまま",0xFFFFFFFF);
		::DrawString(295,320,"君達はジークフロントへ向かう",0xFFFFFFFF);
	}
	if(frame_counter == 900) {
		::DrawString(30,40,"待ちいたるは龍すら届かぬ空の果て",0xFFFFFFFF);
		::DrawString(45,60,"天空の杯に堕ちた神なりし物",0xFFFFFFFF);
	}
	if(frame_counter == 1200) {
		::DrawString(170,400,"計り知れざる苦難を打ち破り",0xFFFFFFFF);
