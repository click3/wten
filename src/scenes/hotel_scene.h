...
class HotelScene : public CampScene {
private:
	void Initialize(void);
public:
	HotelScene(boost::shared_ptr<const std::wstring> default_frame_filename);
	HotelScene(boost::shared_ptr<const Graph> default_frame_graph);
	~HotelScene();

	boost::optional<boost::shared_ptr<Error> > SceneInitialize(void);
	boost::optional<boost::shared_ptr<Error> > StepInitialize(void);
	boost::optional<boost::shared_ptr<Error> > StayStepInitialize(unsigned int bed_lv);
...
private:
	...
	unsigned int bed_lv;
	std::vector<boost::optional<boost::shared_ptr<std::wstring> > > level_up_list;
};
#pragma warning(pop)
