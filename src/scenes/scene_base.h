...
public:
	...
	boost::optional<boost::shared_ptr<Error> > AddSelectorWindow(const std::vector<boost::tuple<boost::shared_ptr<const std::string>, boost::shared_ptr<void> > >& select_list, unsigned int x = 50, unsigned int y = 85, unsigned int width = 540, unsigned int height = 180);
